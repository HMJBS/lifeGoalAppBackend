...
module.exports.removeObject = async function (req, res) {
  ...
  if (owner._id == targetObject.owner) {
    ...
    console.error(`Can't remove object ${targetObject._id}, given user ${targetObject.owner} is not owner ${owner._id}`);
    res.status(400).send(`Can't remove object ${targetObject._id}, given user ${targetObject.owner} is not owner ${owner._id}`);
    return;
  }
  ...
  let deletedObject;
  try {
      deletedObject = await SingleObject.deleteOne({ _id: targetObject._id }).exec();
  } catch (err) {
